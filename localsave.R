...
for (i in 1:num_bands) {
  raster_data <- nc[[i]]                                        # Extract raster data for the current band
...
######################## Parallel processing (saving to local)
num_bands<-19
...
for (i in 1:num_bands) {saveRDS(clipped_vectors_list[[i]], paste0("clip_shp/band_", i, ".rds"))}

# Load clipped vectors
clipped_vectors_list <- list()
for (i in 1:num_bands) {clipped_vectors_list[[i]] <- readRDS(paste0("clip_shp/band_", i, ".rds"))}
clipped_vectors_list[[19]]
plot(clipped_vectors_list[[19]])

################################### for the remaining bands 
library(doParallel)
library (foreach)
num_bands_total <- 79    # Total number of bands
num_bands_processed <-15 # Number of bands already processed
# Start a new cluster
num_cores <- detectCores()
cl <- makeCluster(num_cores)
registerDoParallel(cl)

clipped_vectors_list <- foreach(i = (num_bands_processed + 1):num_bands_total, .packages = c("raster", "sf")) %dopar% {
  raster_data <- nc[[i]]
  vector_data <- rasterToPolygons(raster_data, dissolve = TRUE)
  vector_sf <- st_as_sf(vector_data)
  vector_sf <- st_transform(vector_sf, crs = st_crs(spain))
  clip_vect <- st_intersection(vector_sf, spain)
  names(clip_vect) <- colnames(clip_vect)
  clip_vect <- clip_vect[, 1, drop = FALSE]  #only first column - drop the remaining columns
  return(clip_vect)
}
stopCluster(cl)  # Stop parallel processing

# Save clipped vectors locally for the new bands
for (i in 1:length(clipped_vectors_list)) {saveRDS(clipped_vectors_list[[i]], paste0("clip_shp/band_", (num_bands_processed + i), ".rds"))}

# Load clipped vectors
num_bands<-79
clipped_vectors_list <- list()
for (i in 1:num_bands_total) {file_path <- paste0("clip_shp/band_", i, ".rds")
  # Check if the file exists before attempting to read it
  if (file.exists(file_path)) {clipped_vectors_list[[i]] <- readRDS(file_path)} 
else {warning(paste("File not found:", file_path))}
}
clipped_vectors_list[[1]]
plot(clipped_vectors_list[[10]])